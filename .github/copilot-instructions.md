...
6. **Zero-error 정책 (최우선, 절대 생략 금지)** — 코드 작성·수정 후 반드시 아래 세 가지를 순서대로 **터미널에서 직접 실행**하여 에러가 0개임을 확인한다. 단 하나라도 에러가 있으면 즉시 수정 후 재확인. **터미널 출력을 실제로 읽지 않고 "통과됐을 것"이라고 가정하거나 작업을 마무리해서는 안 된다.**
   ```bash
   npm run typecheck   # TS 컴파일 에러
   npm run lint        # ESLint 에러 (no-floating-promises, no-base-to-string 등)
   npm test            # Vitest 단위 테스트
   ```
   > lefthook pre-commit이 위 세 단계를 순서대로 실행하므로, 로컬에서 미리 통과시키지 않으면 커밋이 막힌다.

   **자주 발생하는 ESLint 에러 패턴 (반드시 준수)**
   | 규칙 | 잘못된 예 | 올바른 예 |
   |------|-----------|-----------|
   | `no-floating-promises` | `chrome.action.setBadgeText({...})` | `void chrome.action.setBadgeText({...})` |
   | `no-unnecessary-type-assertion` | `await sendMessage(...) as { ... }` | `const res: { ... } = await sendMessage(...)` |
   | `no-base-to-string` | `String(unknownVal ?? '')` | `typeof v === 'string' \|\| typeof v === 'number' ? String(v) : ''` |
   | `DOM.Iterable` 누락 | `for (const el of NodeListOf<...>)` | `tsconfig.json lib`에 `"DOM.Iterable"` 추가 |
